...
void updateQuarterCircle(uint8_t dt, uint8_t speed, uint8_t param1, uint8_t param2, Led* leds, uint16_t numLeds)
{
  //param1 = hue
  //param2 = hue2
...
//  printf_("hue: %d, hueChange: %f, speed: %d, cps: %d, dt: %d\n", s.hue.get(), hueChange, speed, cps, dt);
  ...
  const uint16_t ring8 = numLeds / 8;

  const uint8_t sub = 2;

  //back ring
  for(int i = 0; i < ring8 - sub; ++i)
  {
    Buffer::ledBuffer[i].setHue(hue1);
  }
  for(int i = ring4; i < ring4 + ring8 - sub; ++i)
  {
    Buffer::ledBuffer[i].setHue(hue1);
  }

  //front ring
  for(int i = ring2 + ring8; i < ring2 + ring4 - sub; ++i)
  {
    Buffer::ledBuffer[i].setHue(hue2);
  }
  for(int i = ring2 + ring4 + ring8; i < numLeds - sub; ++i)
  {
    Buffer::ledBuffer[i].setHue(hue2);
  }
