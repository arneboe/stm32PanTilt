...
class Fix16 {
	public:
		...
		explicit Fix16(const float inValue)   { value = fix16_from_float(inValue); }
		...
		explicit operator int16_t() const { return fix16_to_int(value);   }
		explicit operator uint8_t() const { return fix16_to_int(value);   }

		uint8_t toUint8() const {return (uint8_t)fix16_to_int(value);}
